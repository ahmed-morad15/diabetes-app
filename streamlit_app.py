...
.chat-message {
    ...
    padding: 10px;
    border-radius: 10px;
    background-color: rgba(255, 255, 255, 0.0); /* شفافية تامة */
}
...
.user-msg {
    ...
    padding-left: 10px;
    background-color: rgba(26, 115, 232, 0.05); /* خلفية خفيفة شفافة */
}
...
def prediction_page():
    ...
    with open('C:/diabetes_prediction_project/env/app/Diabetes_Prediction_Model.pkl', 'rb') as model_file:
        model = pickle.load(model_file)

    data = pd.read_csv('C:/diabetes_prediction_project/env/app/preprocessed_diabetes_data.csv')
    ...
    if submitted:
        ...
        inputs = pd.DataFrame([[age, bmi, HbA1c_level, blood_glucose_level]], columns=['age', 'bmi', 'HbA1c_level', 'blood_glucose_level'])
...
""".format(question=st.session_state.current_question), unsafe_allow_html=True)
    ...
    col0,col0,col1, col2, col3 = st.columns([1,1,1,1,1])
    with col1:
    ...
    col4, col5, col6 = st.columns([1,1.8,1])
    with col5:
    ...
    # Back button
    col7, col8, col9 = st.columns([1,1.3,1])
    with col8:
